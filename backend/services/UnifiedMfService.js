    ...
    if (region === 'india' || region === 'all') {
      try {
        ...
        const mapped = matched.slice(0, 80).map(f => ({
          ...
          nav: f.nav,
          aum: f.aumMetric === 'AUM' ? (f.aumCr ?? f.aum) : null,
          aumCr: f.aumMetric === 'AUM' ? (f.aumCr ?? f.aum) : null,
          aaumCr: f.aaumCr ?? (f.aumMetric === 'AAUM' ? (f.aumCr ?? f.aum) : null),
          aumMetric: f.aumMetric ?? (f.aumCr ? 'AUM' : (f.aaumCr ? 'AAUM' : null)),
      ...
      for (const pid of popularIds) {
        ...
        if (item) {
          found.push({
            ...
            nav: item.nav,
            aum: item.aumMetric === 'AUM' ? (item.aumCr ?? item.aum) : null,
            aumCr: item.aumMetric === 'AUM' ? (item.aumCr ?? item.aum) : null,
            aaumCr: item.aaumCr ?? (item.aumMetric === 'AAUM' ? (item.aumCr ?? item.aum) : null),
            aumMetric: item.aumMetric ?? (item.aumCr ? 'AUM' : (item.aaumCr ? 'AAUM' : null)),
