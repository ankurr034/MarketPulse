...
export const StockDetails = ({ socket }) => {
  ...
  const fetchStockDetails = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(`/api/stocks/${activeSymbol}`);
  ...
  const stats = [
    ...
    { label: '52W High', value: stockMeta.high52 ? formatPrice(stockMeta.high52, activeSymbol) : '—', color: 'var(--gain)' },
    { label: '52W Low', value: stockMeta.low52 ? formatPrice(stockMeta.low52, activeSymbol) : '—', color: 'var(--loss)' },
  ...
  return (
    <div className="view-transition space-y-5">
      ...
      <div className="glass-card p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-5 font-mono text-xs">
          {stats.map((stat) => (
